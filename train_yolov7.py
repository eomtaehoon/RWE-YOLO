...
ROOT = Path(os.path.join(ROOT,'yolov7'))
if str(ROOT) not in sys.path:
...
def train(hyp, opt, device, tb_writer=None):
    ...
    dataloader, dataset = create_dataloader(train_path, imgsz, batch_size, gs, opt,
                                            ...
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '),rwe=(opt.rwe > 0))
    ...
    if rank in [-1, 0]:
        testloader = create_dataloader(test_path, imgsz_test, batch_size * 2, gs, opt,  # testloader
                                       ...
                                       world_size=opt.world_size, workers=opt.workers,
                                       pad=0.5, prefix=colorstr('val: '), rwe = False)[0]
        if opt.rwe > 0:
            testloader_rwe = create_dataloader(test_path, imgsz_test, batch_size * 2, gs, opt,  # testloader
                                       hyp=hyp, cache=opt.cache_images and not opt.notest, rect=True, rank=-1,
                                       world_size=opt.world_size, workers=opt.workers,
                                       pad=0.5, prefix=colorstr('val: '), rwe = True)[0]
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                ...
                if opt.rwe > 0:
                    results_rwe, maps_rwe, times = test.test(data_dict,
                                                 batch_size=batch_size * 2,
                                                 imgsz=imgsz_test,
                                                 model=ema.ema,
                                                 single_cls=opt.single_cls,
                                                 dataloader=testloader_rwe,
                                                 save_dir=save_dir,
                                                 verbose=nc < 50 and final_epoch,
                                                 plots=plots and final_epoch,
                                                 wandb_logger=wandb_logger,
            ...
            if opt.rwe > 0:
                with open(results_file_rwe, 'a') as f:
                    f.write(s + '%10.4g' * 7 % results_rwe + '\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyp.scratch.p5.yaml', help='hyperparameters path')
    ...
    parser.add_argument('--rwe', type=float, default=0.0, help='Row-wise exposure data augmentation probability')
