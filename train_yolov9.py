...
ROOT = Path(os.path.join(ROOT,'yolov9'))

import yolov9.val as validate  # for end-of-epoch mAP
...
from yolov9.utils.plots import plot_evolve, plot_images
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if RANK in {-1, 0}:
        val_loader = create_dataloader(val_path,
                                        ...
                                        rwe = False)[0]
        if opt.rwe > 0:
            val_loader_rwe = create_dataloader(val_path,
                                        imgsz,
                                        batch_size // WORLD_SIZE * 2,
                                        gs,
                                        opt,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                ...
                if opt.rwe > 0:
                    results_rwe, maps_rwe, _ = validate.run(data_dict,
                                                batch_size=batch_size // WORLD_SIZE * 2,
                                                imgsz=imgsz,
                                                half=amp,
                                                model=ema.ema,
                                                single_cls=single_cls,
                                                dataloader=val_loader_rwe,
                                                save_dir=save_dir,
                                                plots=False,
                                                callbacks=callbacks,
                                                compute_loss=compute_loss)
